...
const newsItems = [
  ...
  {
    title:
      "Nvidia reports record sales and beats on earnings again, sending stock higher",
    date: "1 Hour Ago",
    source: "MarketWatch",
...
const Homepage = (props) => {
  ...
  if (timeframe === "1D") {
    ...
    if (timeframe === "1W") {
      for (const dataPoint of large_portfolio_data.slice(large_portfolio_data.length - 8, large_portfolio_data.length)){
        data.push({
          x: dataPoint.date,
          y: parseFloat(dataPoint.price)
        })
      ...
      for (const dataPoint of large_portfolio_data.slice(0, large_portfolio_data.length)) {
        data.push({
          x: dataPoint.date,
          y: parseFloat(dataPoint.price),
        });
    ...
    >
      ...
      >
        <ThemeProvider theme={theme}>
          <Typography
            variant="h6"
            sx={{ marginTop: "2%", marginLeft: "2%", color: "gray" }}
          >
            Invest Value
          </Typography>
          <Typography variant="h3" sx={{ marginLeft: "2%", color: "black" }}>
              ...
              >
                ...
                <TableBody>
                  {activityData.map((row) => (
                    <ActivityRow key={row.datetime} row={row} />
                  ))}
                </TableBody>
              </Table>
            </TableContainer>
        ...
        >
          ...
          >
            ...
            <div style={{display: "flex", width: "100%", justifyContent: "center"}}>
              <Button variant="text" href="/research" sx={{marginTop: "2%"}}>
                View More
              </Button>
            </div>
          </div>
